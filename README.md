...
- [x] 使用 DKG 协议生成阈值密钥材料，确保没有单点持有完整私钥。
- [x] 通过阈值签名，确保用户发起的签名请求都必须得到 n-1 个节点的审计通过才能获得完整签名。
- [x] 使用 Schnorr 知识证明保障用户私钥的安全性，同时用户无法获得完整私钥私自进行签名。
- [] AI 审计网关与内置规则相结合，保证自动审计的严谨性和可解释性，防止滥用和误判。
